...
- `verify:static` now checks both `out/index.html` and `out/work/moneyguard/index.html`. New assertions: route-specific title, canonical and `og:url`, exactly one `h1`, the MoneyGuard heading, all eight content sections, the six workflow steps, the CLI-versus-hosted privacy distinction, the "not anonymous" statement, the substring-method wording in Verification, the source and back links, meaningful pre-JavaScript text volume, absence of an empty shell/spinner/loading placeholder, absence of nineteen banned absolute-privacy and over-claim strings, and the sitemap containing the route. No existing homepage assertion was weakened.
