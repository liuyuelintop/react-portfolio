...
- `aecd1da` followed a second owner review, which found the block flattened: no stack, and no record that the system had ever been deployed. A `Stack studied` line and a past-tense deployment-and-teardown line were added. Removing the stack entirely in `43e8389` was an over-correction on the executor's part; naming the services worked with is a record, not a delivery claim.
- `335458e` corrected a claim of the executor's own making. The bullet `A database verification utility.` credited `backend/database/verify_database.py` to the owner, but that file exists in Ed Donner's public history (`baf951f`, `89353c7`, `d39b8f9`) and was never deleted. The four contribution lines were rewritten so each one traces to a diff on a public branch of `liuyuelintop/ed-ai-in-production-alex`; the guide step is claimed, the script is not.

#### Owner override: the `/work/alex/` route

The original checkpoint brief forbade `/work/alex/`. The owner reversed that after review, on the grounds that a month of infrastructure and agentic-engineering work deserves depth rather than a six-line block. The reversal is recorded here because it inverts two verifier assertions that previously guarded the prohibition.

The route was buildable only after a refactor. `app/work/[slug]/page.jsx` had MoneyGuard's section sequence hardcoded, including `Privacy boundaries` and `Decisions I can defend` — neither of which a study of someone else's architecture can honestly offer. The route now renders an ordered `sections` array in which each entry names a `kind` (`sample`, `prose`, `bullets`, `steps`, `decisions`, `groups`, `details`, `links`, `diagram`), so a case study carries only the sections it can support. MoneyGuard was ported into that shape with no copy change; its existing `verify:static` assertions are the regression net proving the port changed nothing.

`/work/alex/` leads with attribution — the architecture, prompts and Terraform are Ed Donner's, published under MIT, and the page says so before it says anything else — then covers what the study was for, the architecture, the request path, what was deployed and torn down, the four upstream contributions each with its own diff link, and a closing `What this is not`. The architecture diagram is a hand-drawn inline SVG in `src/components/ui/diagrams/AlexArchitectureDiagram.jsx`, built from the repository rather than the course diagrams, with no dashboards, metrics or invented figures. It commits to two facts the previous marketing copy contradicted: the three specialist agents are `@function_tool`s invoked with `InvocationType="RequestResponse"`, so they are sequential rather than simultaneous; and the retrieval side (ingestion Lambda, S3 Vectors, researcher MCP server on App Runner) is not invoked by the analysis path at the commit read, so the drawing joins them with no arrow and says so on its face. The ALEX flagship screenshot is still not used anywhere: it is a screenshot of Ed Donner's UI, and the diagram is the honest visual.

Public ALEX presentation after this checkpoint: title `ALEX — AWS Multi-Agent Architecture Study`, status `Course-based study`, a summary crediting Ed Donner's Udemy capstone and describing the SQS-backed five-role portfolio-analysis workflow and Terraform-defined AWS architecture, a `Stack studied` line, a past-tense deployment-and-teardown line, a `What I contributed` list of the four attributable contributions, and a single link to `/work/alex/`. It has no live link and no modal trigger, and uses the same restrained typography as the Additional Work rows — no screenshot, badge or card. Selected Work stays at MoneyGuard and Melbourne University Ultimate, neither otherwise rewritten. Whether to promote a genuinely-owned third flagship is an open owner decision deferred to a later pass of this release.
...
- `verify:static` now pins the whole homepage work hierarchy in document order (MoneyGuard → Melbourne → Learning & Contributions → ALEX → Stack studied → What I contributed → Read the architecture study → Additional Work) and asserts the title, status, Ed Donner attribution, five-role wording, deployment-and-teardown line and each contribution line verbatim. It rejects a flagship `Read more about … ALEX` modal trigger, the previous flagship title, and the banned claim set including `reducing vector storage cost by ~90%`, `eight Terraform stages`, `least-privilege IAM`, parallel-agent wording and current-deployment wording. It also asserts the ALEX flagship screenshot is still not bundled into the export. Every new assertion was negative-tested against a deliberately mutated build. No existing homepage or MoneyGuard assertion was weakened.
- The two assertions that previously forbade `/work/alex/` and its sitemap entry are inverted under the owner override: the homepage link, the sitemap `<loc>` and the emitted directory list (`alex`, `moneyguard`) are now required. The study page has its own contract: exactly one `h1`, its seven section headings, attribution ordered ahead of the architecture (`Whose project this is` → `Ed Donner` → `MIT licence` → `I did not design this system` → the architecture → `Not my architecture.`), the verbatim `InvocationType="RequestResponse"` and `not simultaneously` statements, `seven Terraform root configurations`, the teardown sentences, the diagram's own labels including `nothing in the analysis path invokes it`, `The verification script itself is Ed's`, and a `compare/main...<branch>` evidence link for each of the three public branches. A page-specific banned list rejects ownership language, concurrency language, `~90%`, `eight terraform stages`, `least-privilege iam` and live-deployment wording.
- Headless Chromium on the exported study page at 390/768/1024/1280/1536 px: no page-level horizontal overflow at any width. The diagram holds a 720 px floor and scrolls inside its own container on narrow viewports, and breaks out of the `max-w-3xl` prose column from `lg` upward so labels never shrink below 11 px.
